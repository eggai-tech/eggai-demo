...
This guide covers optimization techniques for both DSPy-based agents and the
Triage classifier.
...
SIMBA (Simple Instruction Following via Bootstrapping) optimizes agent prompts
automatically. Agents with optimizer scripts:
...
Optimized prompts are saved as JSON files (e.g., `optimized_billing_simba.json`)
and loaded automatically at startup.

## Triage Classifier Optimization

The Triage agent evolved through multiple versions, from basic prompts to
...
**COPRO** (Compositional Preference Optimization) automatically discovers
optimal prompts without training data.
...
**v7 and v8** run entirely locally, making them ideal for privacy-sensitive
deployments.
...
4. **Parallel Track**: v3 (traditional ML) and v5 (neural) for production
   efficiency
...
1. **SIMBA (Individual Agents)**: Best for optimizing ReAct agents with complex
   tool usage
2. **COPRO (Triage v4)**: Zero-shot prompt optimization when you lack training
   data
3. **Few-Shot (Triage v2)**: When you have limited examples but want DSPy
   flexibility
...
[Deployment Guide](multi-environment-deployment.md)
