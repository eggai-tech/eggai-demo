...
async function fetchDatasets(page, limit) {
  ...
  try {
    const response = await fetch(
      `/api/datasets?skip=${(page - 1) * limit}&limit=${limit}`,
    );
    ...
    if (data.datasets.length === 0) {
      datasetsListContainer.innerHTML =
        "<p>No datasets found. Create your first one!</p>";
      return;
    }
    ...
    document.querySelectorAll(".view-examples-btn").forEach((btn) => {
      btn.addEventListener("click", function () {
        ...
        examplesPage = 1;
        document.getElementById("viewExamplesModalLabel").textContent =
          `Dataset Examples (ID: ${datasetId})`;
        fetchExamples(datasetId, examplesPage, examplesPageSize);

        const viewExamplesModal = new bootstrap.Modal(
          document.getElementById("viewExamplesModal"),
        );
...
async function fetchExamples(datasetId, page, limit) {
  ...
  try {
    const response = await fetch(
      `/api/examples?dataset_id=${datasetId}&skip=${
        (page - 1) * limit
      }&limit=${limit}`,
    );
    const data = await response.json();
...
async function prepareExampleForEditing(exampleId) {
  try {
    ...
    const editExampleModal = new bootstrap.Modal(
...
async function updateExample() {
  ...
  try {
    ...
    if (response.ok) {
      const editExampleModal = bootstrap.Modal.getInstance(
        document.getElementById("editExampleModal"),
      );
...
async function createDataset() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const createDatasetModal = bootstrap.Modal.getInstance(
        document.getElementById("createDatasetModal"),
      );
